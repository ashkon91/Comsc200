...
//Assignment 3B Geometry Homework
...
public:
  Square(char* token[4]){
    this->side = token[1]==0?0:atof(token[1]);
  }
  void print() const;
};
...
public:
  Rectangle(char* token[4]){
    this->length = token[1] == 0?0:atof(token[1]);
    this->width = token[2] == 0?0:atof(token[2]);
  }  
  void print() const;
};
...
public:
  Circle(char* token[4]){
    this->radius = token[1] == 0?0:atof(token[1]);
  }
  void print() const;
};
...
public:
  Prism(char* token[4]){
    this->length = token[1] == 0?0:atof(token[1]);
    this->width = token[2] == 0?0:atof(token[2]);
    this->height = token[3] == 0?0:atof(token[3]);
  }
  void print() const;
};
...
public:
  Cylinder(char* token[4]){
    this->radius = token[1] == 0?0:atof(token[1]);
    this->height = token[2] == 0?0:atof(token[2]);
  }
  void print() const;
};
...
int main()
{
  ...
  cout<<"Assignment 3B GeometryHomework2"<<endl;
